/**
 * create_teaser_deck.cjs  –  v3  (full-page layout)
 *
 * Generates a bold, 7-slide Partnership Teaser Deck PDF styled after the
 * Nexalis Health dark-mode landing page & EnterSystem page.
 *
 * v3 changes: content fills the ENTIRE page. Larger fonts, generous paragraph
 * spacing, vertically centered body blocks, and ample breathing room.
 */
...
// ── SLIDE DATA — exact wording preserved ────────────────────────────
const slides = [
    {
...
function measureBody(s, font, boldFont, obliqueFont, bodySz, bodyLH) {
    let totalH = 0;
    for (const para of s.body) {
        if (para === '') { totalH += 16; continue; }
        let f = font; let sz = bodySz;
...
async function build() {
    ...
    const oblique = await doc.embedFont(italicBytes);
    ...
    const profileDims = profileImage.scale(1);
    ...
    for (let idx = 0; idx < slides.length; idx++) {
        ...
        // Grid removed - keeping background pure black
        /*
                const gridStep = 40;
                for (let gx = 0; gx <= W; gx += gridStep) {
                    page.drawLine({
                        start: { x: gx, y: 0 }, end: { x: gx, y: H },
                        thickness: 0.25, color: rgb(1, 1, 1), opacity: 0.025
                    });
                }
                for (let gy = 0; gy <= H; gy += gridStep) {
                    page.drawLine({
                        start: { x: 0, y: gy }, end: { x: W, y: gy },
                        thickness: 0.25, color: rgb(1, 1, 1), opacity: 0.025
                    });
                }
        */

        // ── 3. Gradient accent bar — top ────────────────────────────────
        ...
        // Subliminal glows removed
        /*
                if (s.isTitleSlide) {
                    page.drawCircle({ x: W * 0.25, y: H * 0.50, size: 160, color: BLUE, opacity: 0.05 });
                    page.drawCircle({ x: W * 0.75, y: H * 0.50, size: 160, color: ROSE, opacity: 0.05 });
                    page.drawCircle({ x: W * 0.50, y: H * 0.48, size: 100, color: PURPLE, opacity: 0.04 });
                } else {
                    page.drawCircle({ x: W * 0.7, y: H * 0.4, size: 180, color: BLUE, opacity: 0.03 });
                    page.drawCircle({ x: W * 0.3, y: H * 0.6, size: 120, color: ROSE, opacity: 0.02 });
                }
        */
        ...
        const footSz = 7.5;
        const fw = regular.widthOfTextAtSize(s.footer, footSz);
        page.drawText(s.footer, {
            x: (W - fw) / 2, y: (FOOT_H - footSz) / 2 + 1,
            size: footSz, font: regular, color: SLATE_DIM,
        });

        // ── 6. Page number (top-right) ──────────────────────────────────
        if (!s.isTitleSlide) {
            const numTxt = `${String(idx + 1).padStart(2, '0')} / ${String(slides.length).padStart(2, '0')}`;
            const numW = regular.widthOfTextAtSize(numTxt, 9);
            page.drawText(numTxt, { x: W - MR - numW, y: H - 30, size: 9, font: regular, color: SLATE_DIM });
        }

        // ── available vertical zone ─────────────────────────────────────
        const topY = H - TOP_BAR_H - 40;   // below bar + breathing room
        const botY = FOOT_H + 30;          // above footer + breathing room
        const zoneH = topY - botY;         // ~673 pts of usable space

        // ═══════════════════════════════════════════════════════════════
        //   TITLE SLIDE — everything centered vertically
        // ═══════════════════════════════════════════════════════════════
        if (s.isTitleSlide) {
            ...
            const subLines = wrap(s.subheading, oblique, subSz, USABLE);
            const bodyH = measureBody(s, regular, bold, oblique, bodySz, bodyLH);

            const totalContentH =
                20 +                                        // tag pill
                40 +                                        // gap
                headingLines.length * (headSz + 8) +       // heading
                20 +                                        // accent bar + gap
                subLines.length * (subSz + 6) +            // subheading
                36 +                                        // gap
                bodyH;                                      // body

            let y = botY + (zoneH + totalContentH) / 2;  // start from vertical center

            // Tag — elegant minimal box (white border, blue dot)
            const tagW = regular.widthOfTextAtSize(s.tag, tagSz);
            const padX = 10;
            const padY = 5;
            const boxW = tagW + padX * 2 + 10; // extra space for dot
            ...
            page.drawCircle({ x: boxX + 10, y: boxY + boxH / 2, size: 2.5, color: BLUE });
            page.drawText(s.tag, { x: boxX + padX + 10, y: boxY + padY, size: tagSz, font: regular, color: WHITE_90 });

            // Subtle short line beneath the tag
            const lineW = 30;
            ...
            for (const line of headingLines) {
                const tw = bold.widthOfTextAtSize(line, headSz);
                page.drawText(line, { x: (W - tw) / 2, y, size: headSz, font: bold, color: WHITE });
                y -= headSz + 8;
            }
            ...
            for (const line of subLines) {
                const tw = oblique.widthOfTextAtSize(line, subSz);
                page.drawText(line, { x: (W - tw) / 2, y, size: subSz, font: oblique, color: rgb(0.75, 0.77, 0.80) });
                y -= subSz + 6;
            }
            y -= 28;
            ...
            for (const para of s.body) {
                ...
                const lines = wrap(para, regular, bodySz, USABLE - 40);
                for (const line of lines) {
                    const tw = regular.widthOfTextAtSize(line, bodySz);
                    page.drawText(line, { x: (W - tw) / 2, y, size: bodySz, font: regular, color: rgb(0.85, 0.86, 0.88) });
                    y -= bodyLH;
                }
            }
        ...
        // ═══════════════════════════════════════════════════════════════
        //   CONTENT SLIDES — spread across full page
        // ═══════════════════════════════════════════════════════════════
        const headSz = 32;
        const subSz = 14.5;
        const bodySz = 14;
        ...
        // ── TAG pill (at fixed top position) ────────────────────────────
        let y = topY;
        if (s.tag) {
            const tagSz = 8.5;
            const tagW = bold.widthOfTextAtSize(s.tag, tagSz);
            const padX = 10;
            const padY = 5;
            ...
            page.drawCircle({ x: ML + 10, y: y - 5 + boxH / 2, size: 2.5, color: BLUE });
            page.drawText(s.tag, { x: ML + padX + 10, y: y - 5 + padY, size: tagSz, font: bold, color: WHITE_90 });
            y -= 50;
        }

        // ── HEADING (large, multi-line from \n splits) ──────────────────
        const headingLines = s.heading.split('\n');
        for (const line of headingLines) {
            if (s.isClosingSlide) {
                const tw = bold.widthOfTextAtSize(line, headSz);
                page.drawText(line, { x: (W - tw) / 2, y, size: headSz, font: bold, color: WHITE_90 });
            } else {
                page.drawText(line, { x: ML, y, size: headSz, font: bold, color: WHITE_90 });
            }
            y -= headSz + 6;
        }
        ...
        for (const line of subLines) {
            if (s.isClosingSlide) {
                const tw = oblique.widthOfTextAtSize(line, subSz);
                page.drawText(line, { x: (W - tw) / 2, y, size: subSz, font: oblique, color: rgb(0.78, 0.80, 0.84) });
            } else {
                page.drawText(line, { x: ML, y, size: subSz, font: oblique, color: rgb(0.78, 0.80, 0.84) });
            }
            y -= subSz + 6;
        }

        // ── Calculate remaining space for body & distribute ─────────────
        const bodyTopY = y - 24;
        const bodyBotY = botY;
        const bodyZone = bodyTopY - bodyBotY;
        const naturalBodyH = measureBody(s, regular, bold, oblique, bodySz, bodyLH);

        // Calculate extra spacing to distribute evenly across paragraphs
        const emptyParas = s.body.filter(p => p === '').length;
        const extraSpace = Math.max(0, bodyZone - naturalBodyH);
        // Distribute extra space into paragraph gaps (empty lines)
        ...
        // ── Decorative side accent line (right edge, like a design element) ──
    ...
    const outPath = path.join(
        'C:\\Users\\zSixt\\Desktop\\partnership teaser deck pdf',
        'Nexalis_Partnership_Teaser_Deck_COMBINED.pdf'
    );
